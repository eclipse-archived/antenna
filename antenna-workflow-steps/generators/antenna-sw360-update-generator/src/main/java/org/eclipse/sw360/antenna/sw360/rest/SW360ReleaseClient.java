...
public class SW360ReleaseClient {
    ...
    public List<SW360Release> getReleases(HttpHeaders header) throws IOException, AntennaException {
        HttpEntity<String> httpEntity = RestUtils.getHttpEntity(Collections.emptyMap(), header);
        ...
        if (response.getStatusCode() == HttpStatus.OK) {
            SW360Release<SW360ReleaseLinkObjects, SW360ReleaseEmbedded> resource = response.getBody().getContent();
