...
public class LicenseSelectedRule implements SingleArtifactRule {
    ...
    public Optional<PolicyViolation> evaluate(ThirdPartyArtifact thirdPartyArtifact) {
        if (thirdPartyArtifact.isProprietary() || licenseExpressionOk(thirdPartyArtifact)) {
    ...
    private boolean licenseExpressionOk(ThirdPartyArtifact thirdPartyArtifact) {
        return !thirdPartyArtifact.getLicenseExpression()
                .map(String::toLowerCase)
                .filter(expression -> expression.contains(" or "))
                .isPresent();
    }
