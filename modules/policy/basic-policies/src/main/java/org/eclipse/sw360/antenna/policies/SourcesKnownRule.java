...
public class SourcesKnownRule implements SingleArtifactRule {
    private final Ruleset ruleset;

    public SourcesKnownRule(Ruleset ruleset) {
        this.ruleset = ruleset;
    }

    @Override
    public Optional<PolicyViolation> evaluate(ThirdPartyArtifact thirdPartyArtifact) {
        if (thirdPartyArtifact.isProprietary() || thirdPartyArtifact.getSourceFileOrLink().isPresent()) {
    ...
    public String getName() {
        return "Sources Known Rule";
    }

    @Override
    public String getDescription() {
        return "The artifact sources are not known!";
    }
